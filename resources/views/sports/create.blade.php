                            ...
                            <div class="card-body">
                                ...
                                @foreach ($places as $index => $place)
                                    <div class="form-group row">
                                        <label for="{{ $place['type'] }}-{{ $sport->id }}"
                                               class="col-md-4 col-form-label text-md-right">{{ $place['position'] }}
                                            place:</label>
                                        <input type="hidden" name="score[{{ $sport->id }}][{{ $index }}][type_score]"
                                               value="{{ $index + 1 }}">
                                        <div class="col-md-6">
                                            <select name="score[{{ $sport->id }}][{{ $index }}][country_id]"
                                                    id="{{ $place['type'] }}-{{ $sport->id }}"
